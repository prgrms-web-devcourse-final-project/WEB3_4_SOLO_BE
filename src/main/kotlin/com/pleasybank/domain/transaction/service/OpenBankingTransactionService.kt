package com.pleasybank.domain.transaction.service
...
import com.pleasybank.domain.transaction.dto.TransactionListResponse
...
class OpenBankingTransactionService(
    ...
    fun getDailyTransactionSummary(userId: Long, fintechUseNum: String, date: String): Map<String, Any> {
        ...
        // 수입/지출 분리
        val incomeTrans = transactionList.res_list.filter { transaction -> transaction.inout_type == "I" }
        val outgoingTrans = transactionList.res_list.filter { transaction -> transaction.inout_type == "O" }
        
        // 총 금액 계산
        val totalIncome = incomeTrans.fold(BigDecimal.ZERO) { acc, transaction -> 
            acc.add(BigDecimal(transaction.tran_amt)) 
        }
        
        val totalOutgoing = outgoingTrans.fold(BigDecimal.ZERO) { acc, transaction -> 
            acc.add(BigDecimal(transaction.tran_amt)) 
        }
        
        // 카테고리별 그룹화
        val categoryMap = outgoingTrans.groupBy { transaction ->
            when {
                transaction.print_content.contains("카페") || transaction.print_content.contains("커피") -> "CAFE"
                transaction.print_content.contains("마트") || transaction.print_content.contains("슈퍼") -> "GROCERY"
                transaction.print_content.contains("식당") || transaction.print_content.contains("배달") -> "FOOD"
                transaction.print_content.contains("교통") || transaction.print_content.contains("택시") -> "TRANSPORTATION"
                else -> "OTHER"
            }
        }
    ...
    fun getMonthlyTransactionAnalysis(userId: Long, fintechUseNum: String, year: Int, month: Int): Map<String, Any> {
        ...
        // 일별 그룹화
        val dailyTransactions = transactionList.res_list.groupBy { transaction -> transaction.tran_date }
        
        // 일별 수입/지출 계산
        val dailySummary = dailyTransactions.mapValues { (_, transactions) ->
            val incomes = transactions.filter { transaction -> transaction.inout_type == "I" }
            val outgoings = transactions.filter { transaction -> transaction.inout_type == "O" }
            
            mapOf(
                "income" to incomes.fold(BigDecimal.ZERO) { acc, transaction -> 
                    acc.add(BigDecimal(transaction.tran_amt)) 
                },
                "outgoing" to outgoings.fold(BigDecimal.ZERO) { acc, transaction -> 
                    acc.add(BigDecimal(transaction.tran_amt)) 
                }
            )
        }
        ...
        val categoryOutgoings = transactionList.res_list
            .filter { transaction -> transaction.inout_type == "O" }
            .groupBy { transaction ->
                when {
                    transaction.print_content.contains("카페") || transaction.print_content.contains("커피") -> "CAFE"
                    transaction.print_content.contains("마트") || transaction.print_content.contains("슈퍼") -> "GROCERY"
        ...
        val totalIncome = transactionList.res_list
            .filter { transaction -> transaction.inout_type == "I" }
