...
import com.pleasybank.openbanking.dto.AccountDto
...
class OpenBankingAccountService(
    ...
    fun getAccountByFintechNum(userId: Long, fintechUseNum: String): AccountDetailResponse {
        ...
        // 핀테크 이용번호로 계좌 찾기
        val account = accountListResponse.res_list.find { it.fintech_use_num == fintechUseNum }
    ...
    fun getAccountTransactions(
        ...
        val transactions = transactionList.res_list.map {
            val isIncome = it.inout_type == "I"
            val amount = BigDecimal(it.tran_amt)
            val balanceAfter = BigDecimal(it.after_balance_amt)
            
            val dateFormatter = DateTimeFormatter.ofPattern("yyyyMMdd")
            val timeFormatter = DateTimeFormatter.ofPattern("HHmmss")
            val transactionDateTime = LocalDateTime.parse(
                it.tran_date, dateFormatter
            ).withHour(
                it.tran_time.substring(0, 2).toInt()
            ).withMinute(
                it.tran_time.substring(2, 4).toInt()
            ).withSecond(
                it.tran_time.substring(4, 6).toInt()
            )
    ...
    /**
     * 거래 내용 기반 카테고리 분류 (간단한 키워드 기반)
     */
    private fun categorizeTransaction(description: String): String {
        return when {
            description.contains("급여") || description.contains("월급") -> "SALARY"
            description.contains("카페") || description.contains("스타벅스") || description.contains("커피") -> "CAFE"
            description.contains("마트") || description.contains("슈퍼") -> "GROCERY"
            description.contains("식당") || description.contains("배달") -> "FOOD"
            description.contains("교통") || description.contains("택시") || description.contains("버스") -> "TRANSPORTATION"
            description.contains("통신") || description.contains("핸드폰") -> "COMMUNICATION"
            description.contains("공과금") || description.contains("전기") || description.contains("수도") -> "UTILITIES"
            description.contains("쇼핑") || description.contains("백화점") -> "SHOPPING"
            else -> "OTHER"
        }
    }
} 